 ...
 *
 *  This module represents the main module for the RAMdisk
 *  cfs example program.
 *
 *  This program will manifest a RAMdisk, using the cfs
 *  filesystem library.
 *
...
const   char    *progname = "ramdisk";
const   char    *blankname= "       ";
extern  char    *version;                   // version.c

int             optd;                       // debug flag
int             optm, optM;                 // maximum ramdisk sizes
const   char    *optn;                      // device name (name of ramdisk "device", not usable without a "mount" command)
int             optt;
int             optv;

mpool_t         *mpool_block;               // global (sigh) -- move when we have mount -- holds the ramdisk blocks
...
execute_resmgr (void)
{
    ...
    iofunc_attr_init (&cfs_attr.attr, S_IFNAM | 0666, 0, 0);
...
optproc (int argc, char **argv)
{
    ...
    while ((opt = getopt (argc, argv, "dm:M:n:tv")) != -1) {
        ...
        case    'M':
            ...
            if (floater < 4096) {
                fprintf (stderr, "%s:  ramdisk is too small (must be at least 4k)\n", progname);
                exit (EXIT_FAILURE);
            }
            if (opt == 'm') {
                optm = floater;
...
D           printf ("Selected maximum ramdisk size is %d bytes\n", optm ? optm : optM);
            break;
        case    'n':
            optn = optarg;
            break;
