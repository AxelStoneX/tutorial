 ...
 *
 *  This module is the TAR Filesystem's c_rename handler.
 *
...
D   printf ("%s %d:  cfs_c_rename (%s -> %s)\n", __FILE__, __LINE__, extra -> path, msg -> connect.path);
    ...
    */
    ...
    // (handled separately for easy code removal :-))
    if (sts != ENOENT) {
        return (sts == EOK ? EEXIST : sts);
    }
...
D       printf ("%s %d:  cfs_c_rename, source is missing a non-final component, returns %d (%s)\n", __FILE__, __LINE__, sts, strerror (sts));
    ...
    */
    ...
    // unlink from old... (prolly want better error checking/recovery here...)
