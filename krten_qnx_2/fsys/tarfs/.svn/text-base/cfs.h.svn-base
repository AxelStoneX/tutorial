...
// fes_t is the virtual file
typedef struct fes_s
{
    char                *name;                  // name of the .tar file
    off_t               off;                    // offset from start (size is in attr.attr -> nbytes)
}   fes_t;
// (this really should just point from each cfs_attr_t to a "mount" structure, which can have a cache of whether the file is open or not and an open fd if so...)
...
//      iov_t           *fileblocks;            // nels worth of file blocks
        fes_t           vfile;                  // one (and only one) vfile (filename, offset; length is in attr.nbytes) [see below]
        char            *symlinkdata;           // just a plain, ordinary, strdup'd ASCII string.
    } type;
} cfs_attr_t;

// note that "vfile" is not a pointer to, but rather the structure itself.  That's cuz in order to store the pointer, and malloc the data, we've already used more space
// than what the entire structure cost.
...
cfs_attr_t              *search_dir (char *component, cfs_attr_t *parent);
