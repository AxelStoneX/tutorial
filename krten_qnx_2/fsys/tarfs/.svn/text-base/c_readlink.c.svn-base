 ...
 *
 *  This module is the TAR Filesystem's c_readlink handler.
 *
...
D       printf ("%s %d:  cfs_c_readlink called iofunc_readlink, and it returned %d (%s)\n", __FILE__, __LINE__, sts, strerror (sts));
    ...
    SETIOV (&ctp -> iov [1], target.attr -> type.symlinkdata, strlen (target.attr -> type.symlinkdata));
    msg -> link_reply.path_len = strlen (target.attr -> type.symlinkdata);
