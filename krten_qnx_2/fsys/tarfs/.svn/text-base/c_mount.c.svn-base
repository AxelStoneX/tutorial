 ...
 *
 *  This module is the TAR Filesystem's c_mount handler.
 *
 ...
 *  2003 01 12  R. Krten        extensive modifications for tarfs from ramdisk -- this is, after all, where the .tar file is stuffed and mounted.
...
D   printf ("%s %d:  cfs_c_mount (%s)\n", __FILE__, __LINE__, msg -> connect.path);
    ...
    devocb = (RESMGR_OCB_T *) extra -> extra.srv.ocb;
...
D   printf("Extra type          0x%x\n", msg -> connect.extra_type);
    ...
    */
    if(extra->flags & _MOUNT_ENUMERATE) {
        //Only support the OCB method of enumerating
        if(devocb == NULL) {
            return ENOSYS;
        }
    } else if(!mnt_type || strcmp(mnt_type, OUR_MOUNT_STRING)) {
D       printf ("%s %d:  cfs_c_mount not our type (%s != %s) -> ENOSYS\n", __FILE__, __LINE__, mnt_type, OUR_MOUNT_STRING);
    ...
    if (extra -> flags & _MOUNT_ENUMERATE) {
D       printf ("%s %d:  cfs_c_mount enumerate request not supported yet\n", __FILE__, __LINE__);
        return (ENOTSUP);
    }
    if (extra -> flags & _MOUNT_UNMOUNT) {
D       printf ("%s %d:  cfs_c_mount unmount request not supported yet\n", __FILE__, __LINE__);
        return (ENOTSUP);
    }
    if (extra -> flags & _MOUNT_REMOUNT) {
D       printf ("%s %d:  cfs_c_mount remount request not supported yet\n", __FILE__, __LINE__);
        return (ENOTSUP);
    }
...
CP
    ...
    iofunc_attr_init (&cfs_attr -> attr, S_IFDIR | 0555, NULL, NULL);           // 0555 == seekable, read-only
...
D       printf ("%s %d:  cfs_c_mount called analyze_tar_file (attr %p, name %s) and it wasn't happy, returning %d (%s)\n", __FILE__, __LINE__, cfs_attr, spec_name, ret, strerror (ret));
        return (ret);
    }
