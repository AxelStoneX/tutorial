 ...
 *
 *  This module represents the main module for the SIM-000 driver.
 *
 *  This program will act as a simulator module, generating random values.
 *
 *  2003 10 09  R. Krten        Needed example output for the book, but didn't have the boards any more :-)
...
const   char    *progname = "sim000";
const   char    *blankname= "      ";
extern  char    *version;                   // version.c

int             opt01;                      // default for output is 0 or 1?
...
//  resmgr_attr.msg_max_size = 2048;
    ...
    for (i = 0; i < nsim000; i++) {
        if (resmgr_attach (dpp, &resmgr_attr, sim000 [i].name, _FTYPE_ANY, 0, &connect_func, &io_func, &sim000 [i]) == -1) {
...
optproc (int argc, char **argv)
{
    ...
    if (ph = parser_open (optc)) {
        if (p = parser_parse (ph, "/dev/sim000*")) {
    ...
    if (nsim000 == 0) {         // nothing added by user, add the default one then
        printf ("No configuration specified; using default at port 0x2d0\n");
        install_card (NULL, 0x2d0);
    }
